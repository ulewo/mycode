...
import com.lhl.blog.util.DrowImage;
...
public class ImageUploadServlet extends HttpServlet
{
			...
			IOException
	{
		...
		try
		{
			...
			while (itr.hasNext())
			{
				...
				if (!item.isFormField())
				{
					...
					imgUrl = StringUtil.encodeByMD5(curTime) + "." + imgType;
					String savePath = dataUtl + imgUrl;
					...
					{ // 如果图片太大，就重新画
						DrowImage.saveImageAsJpg(dataUtl + imgUrl, dataUtl + imgUrl, width, 0, false);
					}
				}
			}
		}
		...
		request.setAttribute("result", result);
		request.setAttribute("imgUrl", date + "/" + imgUrl);
		dispatcher.forward(request, response);
	}

	private void createTemp(String realPath)
	{
