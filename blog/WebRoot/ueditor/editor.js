...
UE.commands['insertimage'] = {
    ...
    execCommand:function ( cmd, opt ) {
        ...
        if ( img && /img/i.test( img.tagName ) && img.className != "edui-faked-video" && !img.getAttribute( "word_img" ) ) {
            ...
            ci = opt[0];
            if ( opt.length == 1 ) {
                str = '<img src="' + ci.src + ' " class="imageborder" ' + (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src + '" ' : '') +
                ...
                for ( var i = 0; ci = opt[i++]; ) {
                    str = '<p ' + (ci['floatStyle'] == 'center' ? 'style="text-align: center" ' : '') + '><img src="' + ci.src + '" class="imageborder" ' +
