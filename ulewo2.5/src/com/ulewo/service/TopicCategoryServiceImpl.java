...
public class TopicCategoryServiceImpl extends GroupAuthorityService
		implements
	...
	public void saveCategroy(Map<String, String> map, Integer userId)
		...
		try {
			...
			JSONArray deleteArray = JSONArray.fromObject(URLDecoder.decode(
					deleteRows, "UTF-8"));
			List<TopicCategory> insertList = JSONArray.toList(insertArray,
					new TopicCategory(), new JsonConfig());
			List<TopicCategory> updateList = JSONArray.toList(updateArray,
					new TopicCategory(), new JsonConfig());
			List<TopicCategory> deleteList = JSONArray.toList(deleteArray,
					new TopicCategory(), new JsonConfig());
	...
	private void addCategory(List<TopicCategory> insertList, Integer userId,
	...
	private void updateCategory(List<TopicCategory> updateList, Integer userId,
			String gid) throws BusinessException {
	...
	private void deleteCategory(List<TopicCategory> deleteList, Integer userId,
			String gid) throws BusinessException {
	...
	public TopicCategory getCategroy(Map<String, String> map)
			throws BusinessException {
		String gid = map.get("gid");
		String categoryId = map.get("categoryId");
		if (!StringUtils.isNumber(gid) || StringUtils.isEmpty(categoryId)
				|| !StringUtils.isNumber(categoryId)) {
			throw new BusinessException("参数错误!");
		}
	...
	public void updateCategory(Map<String, String> map, SessionUser sessionUser)
		...
		if (!StringUtils.isNumber(gid)
				...
				|| StringUtils.getRealLength(name) > LengthEnums.Length5
						.getLength() || !StringUtils.isNumber(rang)
	...
	public List<TopicCategory> queryCategoryAndTopicCount(
		...
		return this.topicCategoryMapper
				.selectCategory4ListAndTopicCount(category);
	}
	...
	public UlewoPaginationResult4Json<TopicCategory> queryCategory4Json(
		...
		List<TopicCategory> list = this.topicCategoryMapper
				.selectCategory4ListAndTopicCount(category);
