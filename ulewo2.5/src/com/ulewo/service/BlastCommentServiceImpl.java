...
import com.ulewo.vo.BlastCommentVo;
...
public class BlastCommentServiceImpl implements BlastCommentService {
	...
	public UlewoPaginationResult<BlastComment> queryBlastCommentByPag(
		...
		List<BlastComment> list = this.blastCommentMapper.selectBaseInfoList(
				map, page);
		UlewoPaginationResult<BlastComment> result = new UlewoPaginationResult<BlastComment>(
				page, list);
		return result;
	}
	...
	public Map<String, Object> queryBlastCommentByPag4Api(
		...
		int page_int = 0;
		String pageStr = map.get("pageIndex");
		if (!StringUtils.isEmpty(pageStr) && StringUtils.isNumber(pageStr)) {
			page_int = Integer.parseInt(pageStr);
		}
		int count = blastCommentMapper.selectBaseInfoCount(map);
		...
		List<BlastComment> list = this.blastCommentMapper.selectBaseInfoList(
				map, page);
		List<BlastCommentVo> resultList = new ArrayList<BlastCommentVo>();
		BlastCommentVo commentVo = null;
		for (BlastComment comment : list) {
			commentVo = new BlastCommentVo();
			commentVo.setBlastId(comment.getId());
			commentVo.setId(comment.getId());
			commentVo.setContent(StringUtils.clearHtml(comment.getContent()));
			commentVo.setCreateTime(comment.getShowCreateTime());
			commentVo.setUserIcon(comment.getUserIcon());
			commentVo.setUserId(comment.getUserId());
			commentVo.setUserName(comment.getUserName());
			commentVo.setAtUserId(comment.getAtUserId());
			commentVo.setAtUserName(comment.getAtUserName());
