...
import com.ulewo.vo.TopicCommentVo;
...
public class TopicCommentServiceImpl extends GroupAuthorityService implements
		TopicCommentService {
	...
	public TopicComment addComment(Map<String, String> map,
		...
		if (StringUtils.isEmpty(gid)
				|| !StringUtils.isNumber(gid)
				|| StringUtils.isEmpty(topicId)
				|| !StringUtils.isNumber(topicId)
				|| StringUtils.isEmpty(content)
				|| (!StringUtils.isEmpty(pid) && !StringUtils.isNumber(pid))
				|| (!StringUtils.isEmpty(atUserId) && !StringUtils
						.isNumber(atUserId))) {
		...
		if (TopicCommentTypeEnums.SUBCOMMENT.getValue().equals(
				map.get("commentType"))) {
	...
	public void deleteComment(Map<String, String> map, SessionUser sessionUser)
		...
		if (StringUtils.isEmpty(topicId) || !StringUtils.isNumber(topicId)
				|| StringUtils.isEmpty(idstr)) {
			throw new BusinessException("参数错误");
		}
	...
	public UlewoPaginationResult<TopicComment> queryCommentByTopicId(
		...
		List<TopicComment> list = this.topicCommentMapper.selectPComentList(
				map, page);
		if (null != list && list.size() > 0) {
			map.put("pidStart", String.valueOf(list.get(0).getId()));
			map.put("pidEnd", String.valueOf(list.get(list.size() - 1).getId()));
			List<TopicComment> subList = this.topicCommentMapper
					.selectSubComment(map);
		...
		UlewoPaginationResult<TopicComment> result = new UlewoPaginationResult<TopicComment>(
				page, list);
		return result;
	}
	...
	public UlewoPaginationResult4Json<TopicComment> queryComment4JsonByTopicId(
		...
		SimplePage page = new SimplePage(page_no, count, pageSize);
		List<TopicComment> list = this.topicCommentMapper.selectBaseInfoList(
				map, page);
	...
	public Map<String, Object> queryCommentByTopicId4Api(Map<String, String> map) {
		...
		List<TopicComment> list = this.topicCommentMapper.selectPComentList(
				map, page);
		List<TopicCommentVo> resultList = new ArrayList<TopicCommentVo>();
		int subCount = 0;
		if (null != list && list.size() > 0) {
			map.put("pidStart", String.valueOf(list.get(0).getId()));
			map.put("pidEnd", String.valueOf(list.get(list.size() - 1).getId()));
			...
			for (TopicComment comment : list) {
				...
				vo.setGid(comment.getGid());
				vo.setTopicId(comment.getTopicId());
				vo.setPid(comment.getPid());
				vo.setCreateTime(comment.getShowCreateTime());
				vo.setContent(comment.getSimpleContent());
				vo.setUserId(comment.getUserId());
				vo.setUserName(comment.getUserName());
				vo.setUserIcon(comment.getUserIcon());
				for (TopicComment sub : subList) {
					if (comment.getId().intValue() == sub.getPid().intValue()) {
						TopicCommentVo subVo = new TopicCommentVo();
						subVo.setId(sub.getId());
						subVo.setGid(sub.getGid());
						subVo.setTopicId(sub.getTopicId());
						subVo.setPid(sub.getPid());
						subVo.setCreateTime(sub.getShowCreateTime());
						subVo.setContent(sub.getSimpleContent());
						subVo.setUserId(sub.getUserId());
						subVo.setUserName(sub.getUserName());
						subVo.setUserIcon(sub.getUserIcon());
						subVo.setAtUserIcon(sub.getAtUserIcon());
						subVo.setAtUserId(sub.getUserId());
						subVo.setAtUserName(sub.getUserName());
						vo.getChildList().add(subVo);
						subCount++;
					}
				}
				resultList.add(vo);
		...
		page.setCountTotal(page.getCountTotal() + subCount);
