...
public class UserServiceImpl implements UserService {
	...
	public User register(Map<String, String> map, String sessionCode) throws BusinessException {
		...
		} else if (!StringUtils.checkEmail(email) || StringUtils.isEmpty(email) || email.length() > EMAIL_LENGTH) {
			throw new BusinessException("邮箱地址不符合规范");
		} else if (!StringUtils.checkUserName(userName) || StringUtils.isEmpty(userName.trim())
		...
		} else if (!StringUtils.checkPassWord(password) || StringUtils.isEmpty(password)
	...
	@Override
	public User login(Map<String, String> map, String sessionCode) throws BusinessException {
	...
	@Override
	public User sendRestPwd(Map<String, String> map, String sessionCode) throws Exception, BusinessException {
		String account = map.get("account");
		String checkCode = map.get("code");
	 ...
	 * 重置密码
	 */
	public void resetPwd(Map<String, String> map, String sessionCode) throws BusinessException {
		String account = map.get("account");
		...
		if (!StringUtils.checkPassWord(pwd) || StringUtils.isEmpty(pwd) || pwd.length() < PWD_MIN_LENGTH
	...
	public void updatePassword(Map<String, String> map, SessionUser sessionUser) throws BusinessException {
	...
	@Override
	public User findUser(String value, QueryUserType type) throws BusinessException {
		if (null == value) {
			return null;
		}
		Map<String, String> map = new HashMap<String, String>();
	...
	public void setTheme(Map<String, String> map, SessionUser sessionUser) throws BusinessException {
		String theme = map.get("theme");
		if (!ThemeTypeEnums.TYPE0.getValue().equals(theme) && !ThemeTypeEnums.TYPE1.getValue().equals(theme)) {
	...
	@Override
	public User updateUserInfo(Map<String, String> map, SessionUser sessionUser) throws BusinessException {
		...
		if (address != null && address.trim().length() > MaxLengthEnums.MAXLENGTH50.getLength()) {
			throw new BusinessException("地址信息不能超过50字符");
		}
		if (work != null && work.trim().length() > MaxLengthEnums.MAXLENGTH50.getLength()) {
			throw new BusinessException("工作信息不能超过50字符");
		}
		if (null != character && character.trim().length() > MaxLengthEnums.MAXLENGTH150.getLength()) {
	...
	@Override
	public UlewoPaginationResult findAllUsers(String value, QueryUserType type, int offset, int total) {
	...
	public UlewoPaginationResult findAllUsers(String userName, int page, int pageSize) {

		/*
		 * int count = userDao.findUserCount(userName); Pagination pagination =
		 * new Pagination(page, count, pageSize); pagination.action();
		 * List<User> list = userDao.findAllUsers(userName,
		 * pagination.getOffSet(), pageSize); for (User user : list) {
		 * user.setRegisterTime
		 * (StringUtils.friendly_time(user.getRegisterTime()));
		 * user.setPrevisitTime
		 * (StringUtils.friendly_time(user.getPrevisitTime())); }
		 * PaginationResult result = new PaginationResult(pagination.getPage(),
		 * pagination.getPageTotal(), count, list); return result;
		 */
		return null;
	}
