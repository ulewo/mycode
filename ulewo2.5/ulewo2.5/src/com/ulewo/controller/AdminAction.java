...
public class AdminAction {
	...
	@Autowired
	private BlogCommentService blogReplyService;
	...
	@Autowired
	private BlastService talkService;

	@Autowired
	private BlastCommentService reTalkService;

	@Autowired
	private TopicService articleService;

	@Autowired
	private TopicCommentService reArticleService;
	...
	public Map<String, Object> loadArticle(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			/*
			 * UlewoPaginationResult result =
			 * articleService.queryAllArticleByAdmin(page, pageSize, keyWord);
	...
	public Map<String, Object> loadArticle2(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			/*
			 * UlewoPaginationResult result = articleService
			 * .queryAllArticleByAdmin(page_int, Constant.pageSize25, keyWord);
			 */
	...
	public Map<String, Object> loadReArticle(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			/*
			 * UlewoPaginationResult result =
			 * reArticleService.queryAllReArticle(page, pageSize);
			 * modelMap.put("total", result.getCountTotal());
			 * modelMap.put("rows", result.getList());
			 */
			return modelMap;
	...
	public ModelAndView weekHot(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
		...
		// List<Topic> list = articleService.getArticleInIds(ids);
		ModelAndView mv = new ModelAndView();
	...
	public Map<String, Object> loadUser(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			UlewoPaginationResult<User> result = userService.findAllUsers(userName, QueryUserType.USERID, page,
					pageSize);
			// modelMap.put("total", result.getCountTotal());
			modelMap.put("rows", result.getList());
			return modelMap;
	...
	public Map<String, Object> loadTalk(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			/*
			 * UlewoPaginationResult result =
			 * talkService.queryLatestTalkByPag(page, pageSize);
			 * modelMap.put("total", result.getCountTotal());
			 * modelMap.put("rows", result.getList());
			 */
			return modelMap;
	...
	public Map<String, Object> loadReTalk(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			// UlewoPaginationResult result =
			// reTalkService.queryAllReTalkByPag(page);
			/*
			 * modelMap.put("total", result.getCountTotal());
			 * modelMap.put("rows", result.getList());
			 */
	...
	public Map<String, Object> loadBlog(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			/*
			 * UlewoPaginationResult result =
			 * blogArticleService.queryLatestBlog(page, pageSize);
			 * modelMap.put("total", result.getCountTotal());
			 * modelMap.put("rows", result.getList());
			 */
			return modelMap;
	...
	public Map<String, Object> loadReply(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			/*
			 * UlewoPaginationResult result =
			 * blogReplyService.queryAllReplyByPag(page, pageSize);
			 * modelMap.put("total", result.getCountTotal());
			 * modelMap.put("rows", result.getList());
			 */
			return modelMap;
