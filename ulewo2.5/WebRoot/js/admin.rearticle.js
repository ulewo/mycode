...
//初始化数据
rearticle.loadData = function(){
	$('#dataGrid').datagrid({   
    	url:global.realPath+'/admin/loadReArticle.action',
    	method:'POST',
    	pagination:true, 
    	rownumbers:true,
    	pageSize:20,
	    ...
	    columns:[[ 
	        ...
	        {field:'reTime',title:'发帖时间',width:100}
	    ]]
	});
};


rearticle.deleteReArticle = function(){
	...
	$.messager.confirm("确认","你确定要删除这"+checkedRows.length+"条数据", function (r){  
        if (r) {
        	...
        	$.ajax({
				...
				dataType : "json",
				data : data,
				url : global.realPath+"/admin/deleteReArticle.action",// 请求的action路径
