...
//初始化数据
blogreply.loadData = function(){
	$('#dataGrid').datagrid({   
    	url:global.realPath+'/admin/loadReply.action',
    	method:'POST',
    	pagination:true, 
    	rownumbers:true,
    	pageSize:20,
	    ...
	    columns:[[ 
	        ...
	        {field:'postTime',title:'回复时间',width:100}
	    ]]
	});
};


blogreply.deleteBlogReply = function(){
	...
	$.messager.confirm("确认","你确定要删除这"+checkedRows.length+"条数据", function (r){  
        if (r) {
        	...
        	$.ajax({
				...
				dataType : "json",
				data : data,
				url : global.realPath+"/admin/deleteReply.action",// 请求的action路径
