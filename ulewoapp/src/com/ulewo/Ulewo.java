...
public class Ulewo {
	...
	private static final String BASEUR_ADDCOMMENT = BASEURL
	...
	public static HashMap<String, Object> queryArticle(int articleId) {
		...
		RequestResult requestResult = ApiClient.getUlewoInfo(BASEUR_SHOWARTICLE
				+ "?articleId=" + articleId, NoPage, false, PostType.GET, null,
				null);
		Article article = null;
		if (requestResult.getResultEnum() == ResultEnum.SUCCESS) {
			JSONObject jsonObj = requestResult.getJsonObject();
	...
	public static HashMap<String, Object> queryBlogList(int page) {
		...
		RequestResult requestResult = ApiClient.getUlewoInfo(BASEUR_BLOGLIST
				+ "?page=" + page, page, true, PostType.GET, null, null);
		if (requestResult.getResultEnum() == ResultEnum.SUCCESS) {
			JSONObject jsonObj = requestResult.getJsonObject();
			try {
				JSONObject response = new JSONObject(String.valueOf(jsonObj
	 ...
	 * @return
	 */
	public static HashMap<String, Object> queryReCommentList(int articleId,
			int page) {

		HashMap<String, Object> map = new HashMap<String, Object>();
		String result = Constants.RESULTCODE_SUCCESS;
		List<ReArticle> list = null;
		RequestResult requestResult = ApiClient.getUlewoInfo(BASEUR_RECOMMENT
				+ "?articleId=" + articleId + "&page=" + page, page, true,
				PostType.GET, null, null);
		if (requestResult.getResultEnum() == ResultEnum.SUCCESS) {
			JSONObject jsonObj = requestResult.getJsonObject();
			try {
	...
	public static HashMap<String, Object> login(String userName, String password) {
		...
		RequestResult requestResult = ApiClient.getUlewoInfo(BASEUR_LOGIN, 0,
				true, PostType.POST, params, null);
		User user = null;
		if (requestResult.getResultEnum() == ResultEnum.SUCCESS) {
			JSONObject jsonObj = requestResult.getJsonObject();
			try {
				...
				if (null != response.get("obj")) {
					user = new User(new JSONObject(response.get("obj")
							.toString()));
					map.put("user", user);

					// 将用户名，密码保存到sdk中，将用户名密码设置到内从中
					AppContext
							.putUserInfo(Constants.USERID, user.getUserName());
					AppContext.putUserInfo(Constants.PASSWORD, password);
					AppContext.putUserInfo(Constants.SESSIONID,
							user.getSessionId());
					String str = userName + ";" + password;
					SaveDateThread thread = new SaveDateThread(
							Constants.USERACCOUNT, str.getBytes());
	...
	public static HashMap<String, Object> getInfoFromLocal(String userName,
			String password) {
		...
		if (requestResult.getResultEnum() == ResultEnum.SUCCESS) {
			...
			try {
				...
				AppContext.putUserInfo(Constants.PASSWORD, password);
				AppContext
						.putUserInfo(Constants.SESSIONID, user.getSessionId());
				map.put("user", user);
			} catch (JSONException e) {
				e.printStackTrace();
			}
	...
	public static HashMap<String, Object> addComment(String content,
			int articleId) {
		...
		RequestResult requestResult = ApiClient.getUlewoInfo(BASEUR_ADDCOMMENT,
				0, true, PostType.POST, params, null);
		ReArticle reArticle = null;
		if (requestResult.getResultEnum() == ResultEnum.SUCCESS) {
			JSONObject jsonObj = requestResult.getJsonObject();
			try {
				JSONObject response = new JSONObject(String.valueOf(jsonObj
						.get("response")));
				reArticle = new ReArticle(new JSONObject(response.get("obj")
						.toString()));
