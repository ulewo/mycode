...
public class ReArticleServiceImpl implements ReArticleService {
	...
	public List<ReArticle> queryReArticles(int articleid, int offset, int total) throws Exception {
		...
		/*User author = null;
		User sAuthor = null;
		List<ReArticle> reList = null;
		for (ReArticle reParticle : list) {
			if (Tools.isNotEmpty(reParticle.getAuthorid())) {
				author = userDao.queryUser(null, null, reParticle.getAuthorid());
			}
			else {
				author = new User();
				author.setUserName(reParticle.getAuthorName());
				author.setUserLittleIcon(Constant.USER_DEFAULT_LITTLELICON);
			}
			reParticle.setAuthor(author);
			
			 * reList = reArticleDao.queryReArticleByPid(reParticle.getId());
			 * for (ReArticle reSarticle : reList) { if
			 ...
			 * reSarticle.setAuthor(sAuthor); }
			 * reParticle.setReArticles(reList);
