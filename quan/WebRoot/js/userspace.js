...
function createInfor(data){
	...
	for(var i=0;i<data.length;i++){
		$("<tr>"+
		"<td>"+data[i].title+"</td>"+
		"<td>"+data[i].reNumber+"/"+data[i].readNumber+"</td>"+
...
function pagination(userId,totalNumber,nowPage,pageCount){
	...
	var tds = "<tr>";
	if(curPage>1){
		tds = tds+"<td><a href='javascript:topic(\""+userId+"\",1)'>首&nbsp;&nbsp;页</a></td><td><a href='javascript:topic(\""+userId+"\","+(curPage-1)+")'>上一页</a></td>";
	}
	for(var i = beginNum;i<=endNum;i++){
		if(pageTotal>1){
			if(i==curPage){
				tds = tds+"<td style='background:red'><a href='####'>"+i+"</a></td>";
			}else{
				tds = tds+"<td><a href='javascript:topic(\""+userId+"\","+i+")'>"+i+"</a></td>";	
			}
		}	
	}
	if(curPage<pageTotal){
		tds = tds+"<td><a href='javascript:topic(\""+userId+"\","+(curPage+1)+")'>下一页</a></td><td><a href='javascript:topic(\""+userId+"\","+pageTotal+")'>尾&nbsp;&nbsp;页</a></td>";
	}
	tds = tds+"</tr>";
