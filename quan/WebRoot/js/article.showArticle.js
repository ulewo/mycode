...
function callBackReArticle(data){
	...
	if(authorid==""){
		...
		$("<div class='recon_img'><a href='../user/userInfo.jspx?userId="+authorid+"'><img src='../upload/"+reUserImg+"' width='60' border='0'></a><div>").appendTo(recon_con);
	}
	...
	var recon_info_re = $("<div class='recon_info_re'></div>").appendTo(recon_info);
  	...
  	if(authorid!=""){
  		reUserName = "<a href='../user/userInfo.jspx?userId="+authorid+"'>"+reUserName+"</a>";
  	}
  	var recon_info_info = $("<div class='recon_info_info'>" +
  			"<span class='info_lou'>"+($(".recon_con").length)+"楼</span>" +
...
function showAboutArticle(keyWord,gid){
	$("<img src='../images/loading.gif' id='loadImg'>").appendTo($("#about_con"));
	$.ajax({
		async : true,
		cache : false,
		type : 'POST',
			...
			if(articles.length>0){
				for(var i=0;i<articles.length;i++){
					if($("#title_con").html()!=articles[i].title){
						$("<div class='about_con_info'><a href='post.jspx?id="+articles[i].id+"'>"+articles[i].title+"</a></div>").appendTo($("#about_con"));
					}
				}
			}else{
				$("<div class='about_con_info'>没有找到相关记录</div>").appendTo($("#about_con"));
			}
