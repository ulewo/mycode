function reMarkP(user){
	...
	$("<div class='mark_icon_user'><a href='"+global.realPath+"/user/"+user.userId+"' target='_blank'><img src='"+ global.realPath + "/upload/"
			+user.userLittleIcon+"' width='60'/></a></div>").appendTo(this.mark);
	...
	$("<div class='mark_time_user'>加入时间："+user.registerTime+"</div>").appendTo(this.mark);
}
...
function loadAllUser(){
	$.ajax({
		...
		type : 'GET',
		dataType : "json",
		url : global.realPath+"/selectUsersByMark",// 请求的action路径
